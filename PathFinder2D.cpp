...
bool PathFinder2D::FindPath(int fromX, int fromY, int toX, int toY)
{
        //value is valid
        if(fromX>=cols) fromX =cols-1;
        if(fromY>=rows) fromX =rows-1;
			...
			for (int j = -1; j <= 1; j++)
			{
				...
				if (PointInMap(p.x, p.y, i, j))
				{
					...
					if (bWasHere[p.x + i][p.y + j])
						...
						if (fCurLength + fCost  >= fLengthToPoint[p.x + i][p.y + j])
						{
...
PathFinder2D::Point::Point(PathFinder2D *ref) { x = 0; y = 0; this->ref = ref;}

PathFinder2D::Point::Point(int x, int y,PathFinder2D *ref) { this->x = x; this->y = y;this->ref = ref; }
...
bool PathFinder2D::Point::operator>(const Point& cmp) const
{
	...
	float d1 = sqrt(a1 + b1) + ref->GetLengthToPoint(x,y);


	int a2 = (cmp.x - ref->GetEndX()) * (cmp.x - ref->GetEndX());
	int b2 = (cmp.y - ref->GetEndY()) * (cmp.y - ref->GetEndY());
	float d2 = sqrt(a2 + b2) + ref->GetLengthToPoint(cmp.x,cmp.y);
